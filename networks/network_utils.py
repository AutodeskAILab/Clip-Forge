...
class CResnetBlockConv1d(nn.Module):
    ...
    def __init__(self, c_dim, size_in, size_h=None, size_out=None,
                 norm_method='BatchNorm', legacy=False):
        super().__init__()
        # Attributes
        if size_h is None:
            size_h = size_in
...
class CResnetBlockFC(nn.Module):
    ''' Conditional batch normalization-based Resnet block class Fully Connected.
    Args:
        c_dim (int): dimension of latend conditioned code c
        size_in (int): input dimension
        size_out (int): output dimension
    ...
    def __init__(self, c_dim, size_in, size_h=None, size_out=None,
        ...
        self.bn_0 = CBatchNorm1d_FC(
            c_dim, size_in, norm_method=norm_method)
        self.bn_1 = CBatchNorm1d_FC(
            c_dim, size_h, norm_method=norm_method)
...
class custom_layer_norm(nn.Module):
    def __init__(self, out_ch):
        super().__init__()
        self.layer_norm = nn.LayerNorm([out_ch], elementwise_affine=False)
...
class CBatchNorm1d(nn.Module):
    ...
    def __init__(self, c_dim, f_dim, norm_method='BatchNorm'):
        super().__init__()
        self.c_dim = c_dim
        self.f_dim = f_dim
        ...
        if norm_method == 'BatchNorm':
            ...
            self.bn = custom_layer_norm(f_dim)
        else:
...
class CBatchNorm1d_FC(nn.Module):
    ''' Conditional batch normalization layer class FC.
    Args:
        c_dim (int): dimension of latent conditioned code c
        f_dim (int): feature dimension
        norm_method (str): normalization method
...
class CResnetBlockConv1d_skip(nn.Module):

    def __init__(self, c_dim, size_in, size_h=None, size_out=None,
                 norm_method='BatchNorm', input_dim=3):
        super().__init__()
        # Attributes
        if size_h is None:
            size_h = size_in
        ...
        self.shortcut = nn.Conv1d(input_dim, size_out, 1, bias=False)
        # Initialization
        nn.init.zeros_(self.fc_1.weight)

    def forward(self, x, c, input):
        net = self.fc_0(self.actvn(self.bn_0(x, c)))
        dx = self.fc_1(self.actvn(self.bn_1(net, c)))
        x_s = self.shortcut(input)
        return x_s + dx
    
    
    
class Cond_Sin(nn.Module):
    ''' Conditional Sin normalization layer class.
    Args:
        c_dim (int): dimension of latent conditioned code c
        f_dim (int): feature dimension
        norm_method (str): normalization method
...
class ResnetBlockConv3d(nn.Module):
    ''' 1D-Convolutional ResNet block class.
    Args:
        size_in (int): input dimension
        size_out (int): output dimension
    ...
    def __init__(self, size_in, size_h=None, size_out=None, stride=1, padding=1, kernel=3):
        super().__init__()
        # Attributes
        if size_h is None:
            size_h = size_in
        ...
        self.size_out = size_out
        # Submodules
        self.bn_0 = nn.BatchNorm3d(size_in)
        self.bn_1 = nn.BatchNorm3d(size_h)

        self.fc_0 = nn.Conv3d(size_in, size_h, kernel, padding=padding, stride=stride)
        self.fc_1 = nn.Conv3d(size_h, size_out, kernel, padding=padding, stride=stride)
        self.actvn = nn.ReLU()

        if size_in == size_out:
            self.shortcut = None
        else:
            self.shortcut = nn.Conv3d(size_in, size_out, kernel, bias=False)
